...
def build_agent(env_info, **kwargs):
    ...
    if "hit" in env_info["env_name"]:
        return PPOAgent(env_info, **kwargs, agent_id=1)
    if "defend" in env_info["env_name"]:
        return PPOAgent(env_info, **kwargs, agent_id=1, only_tactic="defend")
    if "prepare" in env_info["env_name"]:
        return PPOAgent(env_info, **kwargs, agent_id=1, only_tactic="prepare")

    return PPOAgent(env_info, **kwargs)
